...
class TestImageMatcher(unittest.TestCase):
    ...
    def test_is_combat_combo_returns_true_if_match(self):
        ...
        _, _, files = next(os.walk(path))
        for file in files:
            self.assertTrue(self._im._is_combat_combo(cv2.imread(path + '/' + file, cv2.IMREAD_COLOR)))

    def test_is_combat_combo_returns_false_if_not_match(self):
        path = '../resources/combatCombo/negatives'
        _, _, files = next(os.walk(path))
        for file in files:
            self.assertFalse(self._im._is_combat_combo(cv2.imread(path + '/' + file, cv2.IMREAD_COLOR)))

    def test_is_start_combat_returns_true_if_match(self):
        path = '../resources/startCombat/positives'
        _, _, files = next(os.walk(path))
        for file in files:
            self.assertTrue(self._im._is_start_combat(cv2.imread(path + '/' + file, cv2.IMREAD_COLOR)))

    def test_is_start_combat_returns_false_if_no_match(self):
        path = '../resources/startCombat/negatives'
        _, _, files = next(os.walk(path))
        for file in files:
            self.assertFalse(self._im._is_start_combat(cv2.imread(path + '/' + file, cv2.IMREAD_COLOR)))
